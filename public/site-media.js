(() => {
  ...
  function enhanceContactPhotoGuide() {
    ...
    const existingGuide =
      form.querySelector('img[src*="greffe capillaire turquie"]') ||
      form.querySelector('img[alt*="photos nécessaires"]') ||
      form.querySelector('img[alt*="photographs required"]');

    if (existingGuide instanceof HTMLImageElement) {
      existingGuide.src = media.photoGuide;
      existingGuide.alt = copy.guideAlt;
      existingGuide.loading = 'lazy';
      existingGuide.decoding = 'async';
      existingGuide.style.width = '100%';
      existingGuide.style.height = 'auto';
      existingGuide.style.borderRadius = '16px';
      existingGuide.style.boxShadow = '0 18px 45px rgba(34, 70, 113, 0.14)';
      return;
    }

    if (document.getElementById('cliniqeo-photo-guide')) return;

    const labels = Array.from(form.querySelectorAll('p, label, h3'));
  ...
  function createResultsGallery() {
    const section = document.createElement('section');
    section.id = 'cliniqeo-results-gallery';
    ...
    media.results.forEach((src, index) => {
      ...
      cardCopy.append(cardTitle, cardCaption);
